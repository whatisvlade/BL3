// ==UserScript==
// @name         strategy_first_date_first_time
...
(function() {
    ...
    // URL, который нужно отслеживать (часть пути запроса слотов)
    ...
    function startOverlayCheckTimer() {
        ...
        overlayCheckTimer = setTimeout(() => {
            ...
            // Проверяем, был ли таймер остановлен
            if (!overlayCheckTimer) {
    ...
    // Выполнить callback, когда оверлей скрыт
    ...
    // Ждём появления элемента по селектору (jQuery)
    ...
    // Ждём, пока календарь подгрузит таблицу
    ...
    // Функция выбора доступной даты (берёт первую доступную после фильтрации)
    function selectAvailableDateInRange(startDate, endDate) {
        ...
        if (availableDates.length > 0) {
            const dateToSelect = availableDates[0]; // первая доступная после фильтрации
