// ==UserScript==
// @name         strategy_last_date_first_time
// @namespace    http://tampermonkey.net/
// @version      2025-02-08
// @description  Скрипт для работы в Mozilla Firefox с Tampermonkey. Автоматизация выбора даты и перехода по календарю с отслеживанием статуса запросов. Добавлена фильтрация «запрещённых» дней месяца (без полного формата YYYY-MM-DD).
// @author       You
// @match        https://appointment.blsspainbelarus.by/Global/Appointment/SlotSelection*
...
(function() {
    ...
    // Функция для отображения сообщения вверху страницы
    function showMessage(message, color) {
        ...
        // Убираем сообщение через 2 секунды, если это не сообщение об ошибке (не красное и не оранжевое)
    ...
    // Функция для проверки, скрыт ли оверлей (global-overlay)
    ...
    // Функция для проверки наличия слотов времени (элементы .slot-item.bg-success)
    ...
    // Функция для запуска таймера проверки оверлея (5 секунд)
    function startOverlayCheckTimer() {
        ...
        overlayCheckTimer = setTimeout(() => {
            ...
            // Если таймер уже был остановлен, прекратить выполнение
            if (!overlayCheckTimer) {
    ...
    // Выполнить callback, когда оверлей скрыт (если нет — сразу, иначе дождаться изменения display)
    ...
    // Ждём появления элемента по селектору (jQuery), возвращаем Promise
    ...
    // Ждём, пока dropdown откроется (aria-expanded === "true")
    ...
    // Ждём, пока dropdown закроется (aria-expanded === "false")
    ...
    // Кликает по переданному элементу, если он существует
    ...
    // Находит виджет календаря и кликает по кнопке открытия
    ...
    // Ждём, пока календарь подгрузит таблицу (<table> внутри .k-calendar-view)
    ...
    // Переход к следующему месяцу (если кнопка не disabled)
    ...
    // Выбор доступной даты (берём последнюю из списка)
    function selectAvailableDateInRange(startDate, endDate) {
        ...
        if (availableDates.length > 0) {
            const dateToSelect = availableDates[availableDates.length - 1]; // берём последнюю доступную дату
    ...
    // Клик по конкретной дате (номер дня)
    ...
    // Открываем dropdown выбора времени после клика по дате
    function findAndOpenDropdown(callback) {
        // Останавливаем таймер проверки оверлея, т.к. мы уже выбрали дату
    ...
    // Выбор первого доступного слота времени
    function selectAvailableTimeSlot() {
        const $slots = $(".slot-item.bg-success").filter(":visible");
        if ($slots.length) {
            const $firstSlot = $slots.first();
            clickElement($firstSlot.get(0));
            console.log("Слот времени выбран:", $($firstSlot).text().trim());
        } else {
    ...
    function redirectToLoginPage() {
        ...
        window.location.href = "https://appointment.blsspainbelarus.by/Global/account/Login?returnUrl=%2FGlobal%2Fappointment%2Fnewappointment&err=HU7zqU0yCxX3GNnx4emgb8d%2FwA73yBclF%2B5Wi%2B0CSYM%3D";
    }

    // Нажатие кнопки Submit для подтверждения выбора слота
    function submitSlotSelection() {
        const $submitButton = $("#btnSubmit").filter(":visible");
        if ($submitButton.length) {
            // Останавливаем таймер проверки оверлея перед нажатием Submit
            stopOverlayCheckTimer();
    ...
    // Основная функция: открываем календарь, переходим к следующему месяцу, выбираем дату и слот
    function openAndSelectDateAndSlot() {
                ...
                waitForCalendarToLoad(function($table) {
                    ...
                    goToNextMonth(function() {
                        runWhenOverlayHidden(function() {
                            ...
                            // Если после фильтрации forbiddenDates нет доступных дат, перенаправляем
        ...
        // Дополнительная проверка: через 25 секунд после запуска, если дата выбрана, но слоты не появились — редирект
        setTimeout(() => {
    ...
    // Перехват fetch-запросов для отслеживания статуса ответа
    ...
    // Перехват XMLHttpRequest для отслеживания статуса ответа
