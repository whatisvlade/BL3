...
(function() {
  ...
  // Улучшенная функция для генерации случайного числа
  ...
  // Улучшенная функция перемешивания массива (Fisher-Yates shuffle)
  ...
  // Улучшенная функция для получения случайного индекса
  function getRandomIndex(arrayLength) {
    if (arrayLength <= 0) return 0;
    return Math.floor(getSecureRandom() * arrayLength);
  }

  function selectAvailableDateInRange(startDate, endDate) {
    let availableDates = findAvailableDatesInRange(startDate, endDate);
    if (availableDates.length > 0) {
      // Перемешиваем массив дат для большей случайности
      availableDates = shuffleArray(availableDates);

      // Выбираем случайный индекс
      const randomIndex = getRandomIndex(availableDates.length);
      const dateToSelect = availableDates[randomIndex];

      selectDate(dateToSelect);
      console.log("Дата выбрана случайно:", dateToSelect, "из", availableDates.length, "доступных дат");
      dateSelected = true;
  ...
  // Улучшенная функция выбора случайного слота времени
  function selectAvailableTimeSlot() {
    const $slots = $(".slot-item.bg-success").filter(":visible");
    if ($slots.length) {
      // Создаем массив всех доступных слотов
      const availableSlots = $slots.toArray();

      // Перемешиваем слоты для большей случайности
      const shuffledSlots = shuffleArray(availableSlots);

      // Выбираем случайный слот
      const randomIndex = getRandomIndex(shuffledSlots.length);
      const selectedSlot = shuffledSlots[randomIndex];

      clickElement(selectedSlot);
      console.log("Случайный слот времени выбран:", $(selectedSlot).text().trim(), "из", $slots.length, "доступных");
    } else {
  ...
  function redirectToLoginPage() {
    ...
    window.location.href = "https://appointment.blsspainbelarus.by/Global/account/login";
  }
